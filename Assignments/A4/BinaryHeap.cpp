...
// Constructor
template <class ElementType> BinaryHeap<ElementType>::BinaryHeap() {}
...
// allocated array
//              maintaining the relative order of the elements and the array is
//              returned
// Postcondition: The Binary Heap remains unchaged
template <class ElementType>
ElementType *
BinaryHeap<ElementType>::getNewSizeArray(unsigned int newCapacity) const {
  ...
  // Copy existing elements from the old elements array to the new elements
  // array
...
void BinaryHeap<ElementType>::reHeapUp(unsigned int indexOfChild) {
  ...
  if (!(elements[indexOfParent] <= elements[indexOfChild])) {
    ...
    // Recursively call reHeapUp on the parent index and fix potential violation
    // of Min Binary Heap
...
template <class ElementType> void BinaryHeap<ElementType>::remove() {
  // Throw an exception if the heap is empty
  if (elementCount == 0)
    throw EmptyDataCollectionException(
        "remove() called with an empty BinaryHeap.");
  ...
  // Resize if elementCount is less than capacity / 4 to maintain efficient use
  // of memory
  if (elementCount <= capacity / 4 && capacity > BASE_CAPACITY) {
    // Always maintain a capacity >= 8 == BASE_CAPACITY, otherwise take half the
    // capacity as new size
...
bool BinaryHeap<ElementType>::insert(ElementType &newElement) {
  ...
  if (elementCount == capacity) {
    ...
    ElementType *newElements =
        getNewSizeArray(newCapacity); // Get resized Array
    if (newElements == nullptr) {
...
ElementType &BinaryHeap<ElementType>::retrieve() const {
  ...
  if (elementCount == 0) {
    ...
    throw EmptyDataCollectionException(
        "Empty Data Collection, no element to retrieve.");
  }
...
// Heap. Postcondition: The Binary Heap remains unchanged. Time Efficiency: O(1)
template <class ElementType>
