...
private:
  constexpr static unsigned int INITIAL_CAPACITY =
      6;                   // Constant INITIAL_CAPACITY
  int *elements = nullptr; // To do: replace this array with a pointer to an
                           // array - see Question 4
  unsigned int elementCount =
      0; // Number of elements in the Queue - if you need it!
  unsigned int capacity =
      INITIAL_CAPACITY; // Initial capacity of the data structure (number of
                        // cells in the array)
  unsigned int frontindex =
      0; // Index of front element (next dequeued/peeked element)
  unsigned int backindex =
      0; // Index of where the next element will be enqueued

  /* You can add private methods. */

  // Utility Methods

  // Description: Makes a deep copy of input object rhs and stores it into this
  // instance Precondition: this->elements is not pointing to heap allocated
  // memory Postcondition: All of rhs data memebers (except elements) are copied
  // into this object
